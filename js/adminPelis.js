...
const tablaPelis = document.createElement("table");
let idMovieSel;
tablaPelis.classList.add("table")
...
`;
rootAdmin.appendChild(tablaPelis);
...
function construirTablaMovies(movies) {
    movies.map(movie => {
        ...
        <td class="table-genero">${movie.genre}</td>
        <td  class="table-categoria">${movie.categorie}</td>
        <td  class="table-año">${movie.year}</td>
...
function allowButtons(e) {
    ...
    idMovieSel = e.target.parentElement.id;
    getMovie(idMovieSel)
        .then(movie => editForm(movie));
}

getMovies()
    .then(movies => construirTablaMovies(movies));
...
`;
 ...
 <div class="modal-body commontexts">
  <h4 id:"modalTextDelet"> ¿Desea eliminar esta pelicula de forma permanente?</h4>
  <p>* Puede desactivar la opción de "Publicada" para ocultar la pelicula sin eliminarla de la base de datos.</p>
</div>
`;
